...
// The wire-path tests live in tests/stack.rs, which cargo runs as its own
// process. Bringing the stack up is a one-way door: bm_l2_deinit is the only
// teardown bm_core exposes, so afterwards a bm_shim_reset frees queues that
// bcmp.c, pubsub.c and the rest still hold pointers to. Keeping those tests
// out of this process is what lets everything here reset freely.
